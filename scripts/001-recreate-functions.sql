-- Recreate all functions with correct column names
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in find_nearby_spots: %', SQLERRM;
        RETURN;
END;
...
BEGIN
    ...
    -- Log the cleanup activity (allow NULL user_id for system operations)
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in cleanup_expired_spots: %', SQLERRM;
        RETURN 0;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in update_user_reputation: %', SQLERRM;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in calculate_parking_demand: %', SQLERRM;
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        RETURN QUERY SELECT 0, 0, 100, 0.0, 0;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in create_ai_prediction: %', SQLERRM;
        RETURN NULL;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Error in update_spot_confidence: %', SQLERRM;
        RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
