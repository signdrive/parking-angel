...
CREATE TABLE IF NOT EXISTS public.parking_spots (
    ...
    geom GEOGRAPHY(POINT, 4326) NOT NULL, -- Use geom instead of location
...
CREATE TABLE IF NOT EXISTS public.user_activities (
    ...
    activity_data JSONB,
    geom GEOGRAPHY(POINT, 4326), -- Use geom instead of location
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_parking_spots_geom ON public.parking_spots USING GIST (geom);
...
CREATE INDEX IF NOT EXISTS idx_ai_predictions_geom ON public.ai_predictions USING GIST (geom);
